	...
	<div class="easyui-layout" data-options="fit:true">	
        <div id="center" data-options="region:'center',border:false" width="90%">
        	<div id="tb" class="datagrid-toolbar">
        		<auth:authenticator action="/admin/rewardtool/rewardtooladd_init">
	        	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="add();">增加</a>
	        	</auth:authenticator>
	        	<auth:authenticator action="/admin/rewardtool/rewardtoolmod_init">
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" onclick="mod();"/>修改</a>
				</auth:authenticator>
				<auth:authenticator action="/admin/rewardtool/rewardtooldel">
    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/rewardtool/rewardtoollist_json',
                columns:[[  
	                {field:'id',title:'ID',width:40},  
              		{field:'nick',title:'道具名称',width:80},
              		{field:'rewardValue',title:'打赏值',width:80},
              		{field:'hotValue',title:'hot值',width:80},
              		{field:'startTimeStr',title:'开始时间',width:200},
              		{field:'endTimeStr',title:'结束时间',width:200},
              		{field:'isClosed',title:'状态',width:40,
              			formatter:function(value,row,index) {return value==1 ? "关闭" : "开放";}},
              		{field:'unit',title:'道具单位',width:100}
            	]]
			});
		});
		...
		function add() {
			$("#listForm").attr("action","<%=contextPath %>/admin/rewardtool/rewardtooladd_init");
			$("#listForm").submit();
		}
		
		function toollist() {
			$("#listForm").attr("action","<%=contextPath %>/admin/rewardtool/rewardtoollist");
			$("#listForm").submit();
		}
		...
		function mod() {
			...
			$("#listForm").attr("action","<%=contextPath %>/admin/rewardtool/rewardtoolmod_init");
			$("#listForm").submit();
		}
		...
		function del() {
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
			    if (r){    
			    	$("#id").attr("value", row.id);
			    	$("#listForm").attr("action","<%=contextPath %>/admin/rewardtool/rewardtooldel");
					$("#listForm").submit();  
			    }    
			});			
		}
