...
<head>
	...
	function loadTree()
	{
		//实例化tree
		$('#tree').tree({  
			url:'<%=contextPath %>/admin/sys/resource_tree',
			onClick:function(node){
				if (node.attributes.nodeType == '${functionNodeType}') {
					var time = new Date().getTime();
					$("#iframe").attr("src","<%=contextPath %>/admin/sys/resource_list?functionId=" + node.id + "&time=" + time);	
				}				
			}
		});	
	}
