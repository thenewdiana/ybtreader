...
<head>
	...
	<script type="text/javascript">
		...
		function add(){
			$("#listForm").attr("action","<%=contextPath %>/admin/sys/function_add_init");
			$("#listForm").submit();
		}
		
		function mod(id){
			$("#id").attr("value",id);
			$("#listForm").attr("action","<%=contextPath %>/admin/sys/function_mod_init");
			$("#listForm").submit();
		}
	</script>
</head>
...
<body class="easyui-layout" fit="true">
	...
	<div region="center" id="center" border="false">
		<div id="rowBar" style="display:none;text-align:right;">	
			<auth:authenticator action="/admin/sys/function_add_init">
		...
		<table id="listTable">
			<thead style="display:none">  
		        <tr>  
		            ...
		            <th data-options="field:'name',width:$(this).width()*0.6,resizable:false">功能名</th>  
		   	...
		   	<tbody style="display:none">
		   		<c:forEach items="${list }" var="item">
		   			<tr>
						...
						<td>${item.name }</td>
						<td>
							<auth:authenticator action="/admin/sys/function_mod_init">
