	...
	<div class="easyui-layout" data-options="fit:true">	
        <div id="center" data-options="region:'center',border:false" width="90%">
        	<div id="tb" class="datagrid-toolbar">
        		<auth:authenticator action="/admin/androidUpdate/add_init">
    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/androidUpdate/list_json',
                columns:[[  
	                ...
	                {field:'versionNumber',title:'版本序号',width:$(this).width()*0.2},
              		...
              		{field:'appName',title:'app名称',width:$(this).width()*0.2},
              		{field:'coerceUpdate',title:'是否强制升级',width:$(this).width()*0.1,formatter:formatSelect},
              		{field:'oper', title:'操作', width:$(this).width()*0.2, formatter:function(value,row,index){
	                       var id = row._id; 
	                       var link = '<auth:authenticator action='/admin/androidUpdate/edit_init'><a class="editClass" href="javascript:mod(\'' + id + '\');">编辑</a> </auth:authenticator>';
	                       link += '<auth:authenticator action='/admin/androidUpdate/del'><a class="delClass" href="javascript:del(\'' + id + '\');">删除</a> </auth:authenticator>'; 
	                       return link;
                    	}
                    }
            	]],
		...
		function add() {
			$("#listForm").attr("action","<%=contextPath %>/admin/androidUpdate/add_init");
			$("#listForm").submit();
		}
		
		function mod(id){
		   	location.href = "<%=contextPath%>/admin/androidUpdate/edit_init?id=" + id;
		}
		
		function del(id){
		    var ajaxUrl = "<%=contextPath%>/admin/androidUpdate/del";
