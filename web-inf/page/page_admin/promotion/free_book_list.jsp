...
<body>
	<div class="easyui-layout" data-options="fit:true">
		<div id="north" title="限免时间"  style="background-color: #E0ECFF;height:60px;"
		...
		<div id="center" data-options="region:'center',border:false" width="90%">
        	<form id="searchForm" name="searchForm" action="<%=contextPath%>/admin/free/list_json" method="post">
				
			</form>
        	<div id="buttons" class="toolbar-button">
	        	<auth:authenticator action='/admin/free/add_init'>
	        	<a href="javascript:add();" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">添加</a> 
	        	</auth:authenticator>
	        	<auth:authenticator action='/admin/free/saveBookList'> 
	...
	<div id="time_dlg" class="easyui-dialog" title="修改限免有效期" style="width:380px;height:350px;"
		 data-options="
                iconCls: 'icon-edit',
                closed: true,
    ...
    <script type="text/javascript">
		$(function(){
			$("#bookList").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/free/list_json?noReset=1',
                columns:[[  
	                ...
	                {field:'sort',title:'显示顺序',width:120},
              		...
              		{field:'oper', title:'操作', width:120, formatter:function(value,row,index){
	                       var id = row.id; 
	                       var link = '<auth:authenticator action='/admin/free/mod_init/'><a class="editClass" href="javascript:mod(' + id + ');">编辑</a> </auth:authenticator>';
	                       link += '<auth:authenticator action='/admin/free/del/'><a class="delClass" href="javascript:deleteBook(' + id + ');">删除</a> </auth:authenticator>'; 
	                       return link;
                    	}
                    }
            	]],
        ...
        function add(){
        	location.href = "<%=contextPath%>/admin/free/add_init";
        }
		
		function mod(id){
		   	location.href = "<%=contextPath%>/admin/free/mod_init/" + id;
		}
		
		function deleteBook(id){
		    var ajaxUrl = "<%=contextPath%>/admin/free/del";
        ...
        function showDlg(){
            $.ajax({
                type:"POST",
                url: "<%=contextPath%>/admin/promotion/mod_init/free",
                dataType : "json",
        ...
        function saveTime(){
            ...
            $.ajax({
                type:"POST",
                url: "<%=contextPath%>/admin/promotion/mod/free",
