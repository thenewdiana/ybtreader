...
<body>
	<div class="easyui-layout" data-options="fit:true">	
		<div id="north" title="搜索条件"  style="background-color: #E0ECFF;height:60px;"
	       ...
	       <form id="searchForm" name="searchForm" action="<%=contextPath%>/admin/statistics/subscribe_list_json" method="post">
	       	  <table>
		         <tr>
		           	<th>书名：</th>
		            ...
		            <auth:authenticator action="/admin/statistics/exportSubscribeList">
    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/statistics/subscribe_list_json',
                columns:[[  
    	...
    	function byDay(id,statisticalDate) {
			...
			$("#listForm").attr("action","<%=contextPath %>/admin/statistics/subscribe_day_list");
			$("#listForm").submit();
		}
		
		function exportExcel() {
			$("#searchForm").attr("action","<%=contextPath %>/admin/statistics/exportSubscribeList");
			$("#searchForm").submit();
		}
	</script>
</body>
