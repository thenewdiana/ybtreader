    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/activity/rulelist_json?activityId=${activityId}',
                columns:[[  
	                {field:'id',title:'ID',width:60},  
              		{field:'name',title:'规则名称',width:300},
              		{field:'minValue',title:'最小充值糖豆数（含）',width:150},
              		{field:'maxValue',title:'最大充值糖豆数（不含）',width:150},
              		{field:'giftValue',title:'赠送糖豆数',width:150}
            	]]
			});
		});
		...
		function add() {
			$("#listForm").attr("action","<%=contextPath %>/admin/activity/ruleadd_init");
			$("#listForm").submit();
		}
		
		function toollist() {
			$("#listForm").attr("action","<%=contextPath %>/admin/activity/rulelist");
			$("#listForm").submit();
		}
		...
		function mod() {
			...
			$("#listForm").attr("action","<%=contextPath %>/admin/activity/rulemod_init");
			$("#listForm").submit();
		}
		...
		function del() {
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
			    if (r){    
			    	$("#id").attr("value", row.id);
			    	$("#listForm").attr("action","<%=contextPath %>/admin/activity/ruledel");
					$("#listForm").submit();  
			    }    
			});			
		}
