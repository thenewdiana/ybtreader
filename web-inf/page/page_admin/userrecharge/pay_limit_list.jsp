	...
	<div class="easyui-layout" data-options="fit:true">	
        <div id="center" data-options="region:'center',border:false" width="90%">
        	<div id="tb" class="datagrid-toolbar">
        		<auth:authenticator action="/admin/activity/payLimitadd_init">
	        	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="add();">增加</a>
	        	</auth:authenticator>
				<auth:authenticator action="/admin/activity/payLimitdel">
    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/activity/payLimitlist_json',
                columns:[[  
	                {field:'id',title:'ID',width:40}, 
	                {field:'minValue',title:'最小限制金额(元)',width:200},
              		{field:'maxValue',title:'最大限制金额(元)',width:200}	
            	]]
			});
		});
		...
		function add() {
			$("#listForm").attr("action","<%=contextPath %>/admin/activity/payLimitadd_init");
			$("#listForm").submit();
		}
		...
		function del() {
			...
			$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
			    if (r){    
			    	...
			    	$("#listForm").attr("action","<%=contextPath %>/admin/activity/payLimitdel");
					$("#listForm").submit();  
			    }    
			});			
		}
