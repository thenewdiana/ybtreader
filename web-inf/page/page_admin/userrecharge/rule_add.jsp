...
<head>
    ...
    <script type="text/javascript">
		function submitForm(){
	    	...
	    	$("#addForm").attr("action","<%=contextPath %>/admin/activity/ruleadd");	    	
	    	$('#addForm').submit();
	    }
		
		function goback(){
			window.location = "<%=contextPath %>/admin/activity/rulelist?activityId=${activityId}";	
		}
	</script>
</head>
		...
		<div class="easyui-panel" title="投票活动增加" style="width:750px;height:auto">			
			...
			<table class="promodify" cellspacing="1">
				...
				<tr>
					...
					<td>
						<input type="text" id="minValue" name="minValue" class="input-text easyui-textbox" data-options="required:true"/>
					</td>
				</tr>
				<tr>
					<td class="info">充值糖豆数上限（不含）：</td>
					<td>
						<input type="text" id="maxValue" name="maxValue" class="input-text easyui-textbox" data-options="required:true"/>（0表示不限上限）
					</td>
				</tr>	 
				<tr>
					<td class="info">免费赠送糖豆数：</td>
					<td>
						<input type="text" id="giftValue" name="giftValue" class="input-text easyui-textbox" data-options="required:true"/>(负数表示按比例赠送，数值是赠送比例的10倍。如-1表示赠送充值数的10%，-10表示赠送充值数的1倍，-20表示赠送充值数的2倍)
					</td>
				</tr>	
			</table>
			</form>		
