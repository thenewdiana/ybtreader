	...
	<div class="easyui-layout" data-options="fit:true">	
        <div id="center" data-options="region:'center',border:false" width="90%">
        	<div id="tb" class="datagrid-toolbar">
        		<auth:authenticator action="/admin/appStart/add_init">
    ...
    <script type="text/javascript">
		$(function(){
			$("#list").datagrid({
			    ...
			    striped: true, 
			    singleSelect:true,
			    url:'<%=contextPath%>/admin/appStart/list_json',
                columns:[[  
	                {field:'id',title:'ID',width:100,width:$(this).width()*0.1}, 
              		{field:'title',title:'标题',width:$(this).width()*0.2},
              		{field:'url',title:'跳转url',width:$(this).width()*0.3},
              		{field:'inside',title:'是否内链',width:$(this).width()*0.1,formatter:formatSelect},
              		{field:'isPublic',title:'是否发布',width:$(this).width()*0.1,formatter:formatSelect},
              		{field:'oper', title:'操作', width:$(this).width()*0.2, formatter:function(value,row,index){
	                       var id = row.id; 
	                       var link = '<auth:authenticator action='/admin/appStart/edit_init'><a class="editClass" href="javascript:mod(' + id + ');">编辑</a> </auth:authenticator>';
	                       link += '<auth:authenticator action='/admin/appStart/del'><a class="delClass" href="javascript:del(' + id + ');">删除</a> </auth:authenticator>'; 
	                       return link;
                    	}
                    }
            	]],
		...
		function add() {
			$("#listForm").attr("action","<%=contextPath %>/admin/appStart/add_init");
			$("#listForm").submit();
		}
		
		function mod(id){
		   	location.href = "<%=contextPath%>/admin/appStart/edit_init?id=" + id;
		}
		
		function del(id){
		    var ajaxUrl = "<%=contextPath%>/admin/appStart/del";
