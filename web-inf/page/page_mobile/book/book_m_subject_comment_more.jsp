	...
	<c:forEach items="${commentList }" var="item" varStatus="status">
		<div class="dis_pl_outer clearfix">
    		<dl class="dis_o clearfix">
	      		<dt>
	      			<span class="col898989">${item.userNick }</span><span class="b_num v_${item.vipValues.id }"></span><span class="dis_icon ${item.fansValues.mobileCss }" title="${item.fansValues.name }">${item.fansValues.name }</span>
	      		</dt>
    		...
    		<p class="pt5"><span class="f14 cola21812">${item.title }</span></p>
  		...
  		<c:if test="${item.replyCommentList.size() > 0 }">
  			...
  			<dl class="pl_zt_dl clearfix">
    			<dt >
    				<span class="col0099cc">${replyItem.userNick }</span><span class="b_num v_${replyItem.vipValues.id }"></span><span class="dis_icon ${replyItem.fansValues.mobileCss }">${replyItem.fansValues.name }</span>
    				：${replyItem.content }
    			</dt>
    			<dd >
...
<form action="${pageContext.request.contextPath }/m/book/${book.id}/${commentId }/comment/more" method="post" id="bookCommentForm">
...
function isLogin() {
	...
	if (userId == null || userId == "") {
		var redirectUrl = "/m/book/${book.id}/${commentId}/comment/more";
