	...
	<c:forEach items="${commentList }" var="item" varStatus="status">
		<div class="dis_pl_outer clearfix">
    		<dl class="dis_o clearfix">
	      		<dt>
	      			<a href="${pageContext.request.contextPath }/m/userCenter/${item.userId }"><span class="col898989">${item.userNick }</span></a>
	      			<span class="b_num v_${item.vipValues.id }"></span>
	      			<span class="${item.fansValues.mobileCss }" title="${item.fansValues.name }">${item.fansValues.name }</span>
	      		</dt>
	      		<dd>&nbsp;${item.createTimeFormat }&nbsp;<a href="${pageContext.request.contextPath }/m/book/comment/reply?callBackUrl=m/book/${book.id}/chapter/comment/more&commentId=${item.id}">回复</a> </dd>
    		</dl>
    		<!--  
    		<p class="col898989">评《${book.name }》</p>
    		-->
  		...
  		<c:if test="${item.replyCommentList.size() > 0 }">
  			...
  			<dl class="pl_zt_dl clearfix">
    			<dt >
    				<a href="${pageContext.request.contextPath }/m/userCenter/${replyItem.userId}"><span class="col0099cc">${replyItem.userNick }</span></a>
    				<span class="b_num v_${replyItem.vipValues.id }"></span>
    				<span class="dis_icon ${replyItem.fansValues.mobileCss }">${replyItem.fansValues.name }</span>
    				：${replyItem.content }
    			</dt>
    			<dd >
...
<div class="goback">
<a class="homes" href="${pageContext.request.contextPath }/m"><img height="16" width="18" alt="返回首页" src="${sessionScope.baseUrl}mobile/images/home.png"></img></a>
&gt;<a href="${pageContext.request.contextPath }/m/book/${book.id}" class="col992123"><em></em>${book.name }</a></div>
<page:mpagination pageAttr="pagination"></page:mpagination>
<!--footer-->
<%@ include file="../footer_m.jsp"%>
<form action="${pageContext.request.contextPath }/m/book/${book.id}/chapter/comment/more" method="post" id="bookCommentForm">
...
function isLogin() {
	...
	if (userId == null || userId == "") {
		var redirectUrl = "/m/book/${book.id}/chapter/comment/more";
