...
<div class="ranking"><h1>写《${book.name }》的评论</h1></div>
    <section class="alipay enroll discuss">
    	...
    	<input type="hidden" name="replyId" value="${bookComment.id }"/>
        <input type="hidden" name="title" value="${bookComment.title }"/>
        <input type="hidden" name="replyContent" value="${bookComment.content }"/>
        <input type="hidden" name="replyUserId" value="${bookComment.userId }"/>
        <div class="ce-c">
                <dl>
                    <dt>回复：</dt>
                    <c:choose>
                    	<c:when test="${bookComment.title == null }">
                    		<dd>${bookComment.content }</dd>
                    	</c:when>
                    	<c:otherwise>
							<dd>${bookComment.title }</dd>                    	
                    	</c:otherwise>
                    </c:choose>
                </dl>
                <dl>
                    <dt>内容</dt>
                    <dd>
                        <%--<textarea class="textarea1 disinput" name="content">回复@${user.nick }：</textarea>--%>
                        <textarea class="textarea1 disinput" name="content"></textarea>
                    </dd>
                </dl>
...
$(function(){
	...
	$("#btn_comment").bind("click", function() {
		...
		$.ajax({
			...
			data : formData,
			async:true,
			url : "${pageContext.request.contextPath }/m/book/comment/reply",
