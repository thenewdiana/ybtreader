...
<!--中间内容-->
<form action="<%=contextPath%>/m/userCenter/shelf/${userId}" id="shelfForm" method="post">
	<div class="comment book2">
		...
		<c:if test="${not empty userBookList}">
			<c:forEach items="${userBookList}" var="item" varStatus="status">
				<dd>
		        	<h2><a href="${contextPath }/m/book/${item.bookId}" title="${item.bookName}">${item.bookName}</a></h2>
		            <p class=""><a href="${contextPath }/m/book/${item.bookId}/${item.lastChapterId}">${item.lastChapterTitle}&nbsp;</a> <time class="mdhm">${item.modifyTime}</time></p>
		            <a href="${contextPath }/m/book/${item.bookId}" title="开始阅读" class="read1">开始阅读</a>
		        </dd>
			</c:forEach>
		</c:if>
